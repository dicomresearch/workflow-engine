...
namespace dicom\workflow\building\expressions\creation;

use dicom\workflow\building\expressions\creation\exceptions\ExpressionMappingException;

class ExpressionAliasMapping
{
    private static $registeredExpressionAliases = [
        'currentdate' => 'dicom\workflow\engine\expressions\CurrentDateExpression',
    ];
    ...
    public static function getAliasByClassName($className)
    {
        if (!in_array($className, static::$registeredExpressionAliases)) {
            throw ExpressionMappingException::cantFindAliasForExpression(
                $className,
                static::$registeredExpressionAliases
            );
        }
    ...
    public static function addAlias($alias, $className)
    {
        ...
        if (array_key_exists($alias, static::$registeredExpressionAliases)) {
            throw ExpressionMappingException::aliasAlreadyRegistered(
                $alias,
                $className,
                static::$registeredExpressionAliases
            );
